...
const InfoRow = ({
	...
	<div className='flex items-start gap-3'>
		<span className='mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-rccg-red/10'>
...
export const EventDetail = () => {
	...
	if (isError) {
		return (
			<div className='flex flex-col items-center gap-4 rounded-2xl bg-amber-50 py-16 text-center ring-1 ring-amber-200'>
				...
				<Link
					to='/events'
					className='inline-flex items-center gap-1.5 text-sm font-bold text-rccg-red no-underline hover:underline'
				>
	...
	return (
		<div className='space-y-10 sm:space-y-12'>
			...
			<Link
				to='/events'
				className='inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 no-underline transition hover:text-rccg-red'
			>
				<ArrowLeft className='h-3.5 w-3.5' aria-hidden />
				All Events
			</Link>
			...
			<section className='relative overflow-hidden rounded-2xl bg-slate-900 text-white shadow-xl'>
				...
				<div className='relative z-10 px-6 py-16 sm:px-10 sm:py-20'>
					<div className='flex flex-wrap items-center gap-3'>
						{data.category && (
							<span className='inline-block rounded-full bg-rccg-red px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-white'>
								{data.category}
							</span>
						)}
						{isPast && (
							<span className='inline-block rounded-full bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-300'>
								Past Event
							</span>
						)}
					</div>
					...
					{data.startDate && (
						<p className='mt-5 flex items-center gap-2 text-sm text-rccg-gold'>
			...
			<section className='grid gap-8 lg:grid-cols-[1fr_320px] xl:grid-cols-[1fr_360px]'>
				...
				<div className='space-y-6'>
					...
					{/* Rich body content (from Sanity portable text — plain fallback) */}
					{data.excerpt && (
				...
				<aside className='space-y-5'>
					...
					<div className={`overflow-hidden rounded-2xl shadow-md ${isPast ? 'bg-slate-50 ring-1 ring-slate-200' : 'bg-rccg-maroon text-white'}`}>
						<div className='px-6 py-6'>
							...
							<h3 className={`mt-3 text-base font-extrabold ${isPast ? 'text-slate-800' : 'text-white'}`}>
								{isPast ? 'This event has passed' : "You're Invited"}
							</h3>
							<p className={`mt-2 text-sm leading-relaxed ${isPast ? 'text-slate-500' : 'text-white/75'}`}>
								{isPast
							...
							{data.registrationLink && !isPast && (
								<a
									...
									className='mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full bg-rccg-gold py-3 text-xs font-bold uppercase tracking-[0.16em] text-rccg-maroon no-underline shadow transition hover:bg-amber-400'
								>
									Register Now
							...
							{isPast && (
								<Link
									to='/events'
									className='mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-rccg-red no-underline hover:underline'
								>
									See upcoming events
					...
					<div className='rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200'>
						<h3 className='text-sm font-bold text-slate-800'>Need more info?</h3>
					...
					<div className='rounded-2xl bg-rccg-red/5 p-5 ring-1 ring-rccg-red/10'>
						<p className='text-xs font-semibold text-rccg-red'>
						...
						<Link
							to='/service-times'
							className='mt-3 inline-flex items-center gap-1.5 text-xs font-bold text-rccg-red no-underline hover:underline'
						>
							All service times <ArrowRight className='h-3 w-3' aria-hidden />
						</Link>
					</div>
				</aside>
			</section>
